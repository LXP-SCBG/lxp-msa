...
/**
 * 강의 카운터 행(lecture_seats) 생성 전용 컴포넌트.
 *
 * <p>INSERT IGNORE 를 수강 신청 트랜잭션 안에서 실행하면, 중복 키 체크 과정에서
 * 잡힌 공유 락(S)이 트랜잭션 끝까지 유지되어 이어지는 차감 UPDATE(X락 요청)와
...
public class LectureSeatInitializer {
    ...
    /** 카운터 행이 없으면 잔여석 초기값(= 강의 정원)으로 생성한다. */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void createIfAbsent(Long lectureId, int maxEnrollment) {
        lectureSeatRepository.insertIgnore(lectureId, maxEnrollment);
    }
}
